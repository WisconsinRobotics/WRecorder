...
if __name__ == "__main__":
	...
	if len(sys.argv) != 4:
		raise ValueError(
			'Invalid number of argumets. Must inclue "-ip", "-port" and "-video"!'
		)
	...
	for arg in sys.argv[1:]:
		if arg.startswith("-ip="):
			...
			raise ValueError(
				'Invalid argument(s). Must be "-ip" or "-port" or "-video"!'
			)
	...
	os.system(
		f'gst-launch-1.0 v4l2src device="/dev/video{video}" ! video/x-raw, width=640, height=480 ! videoconvert ! openh264enc bitrate=500000 ! rtph264pay config-interval=1 ! udpsink host={ip} port={port} sync=false async=false'
	)

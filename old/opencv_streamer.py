...
SUBNET_MASK = (
	subprocess.check_output("ip addr show", shell=True)
	.decode()
	.split(f"inet {BROADCAST_IP}/")[-1]
	.split(" ")[0]
)
DISCOVERY_IP = ipaddress.IPv4Network(
...
def broadcast_ip(discovery_port, discovery_timeout):
	...
	while connection_countdown > 0:
		broadcast_socket.sendto(
			f"IP_BROADCASTER:tcp://{BROADCAST_IP}:{discovery_port}".encode("utf8"),
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(
		prog="opencv_streamer", description="Streams camera data using opencv2"
	)
	...
	if auto_ip_discovery:
		print(
			f"Broadcasting IP on port {discovery_port} for {discovery_timeout} seconds..."
		)
